...
/**
 * OTP step for forgot-password only. Sign-up continues to use {@link OtpActivity}.
 */
public class ResetPasswordOtpActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        resetLoginId = getIntent().getStringExtra(ResetPasswordActivity.EXTRA_EMAIL);
        resetUserId = getIntent().getStringExtra(ResetPasswordActivity.EXTRA_USER_ID);
    ...
    private void verifyOtp() {
        ...
        FirebaseUtils.signInWithPhoneCredential(credential, true, (@NonNull Task<AuthResult> task) -> {
            btnSubmit.setEnabled(true);
            if (task.isSuccessful()) {
                Intent intent = new Intent(this, ResetPasswordActivity.class);
                intent.putExtra(ResetPasswordActivity.EXTRA_EMAIL, resetLoginId);
                intent.putExtra(ResetPasswordActivity.EXTRA_USER_ID, resetUserId);
                intent.putExtra(ResetPasswordActivity.EXTRA_OTP_VERIFIED, true);
                startActivity(intent);
                finish();
                return;
            }
            String detail = task.getException() != null ? task.getException().getMessage() : "";
            android.util.Log.e("ResetPasswordOtp", "Phone credential failed: " + detail);
            Toast.makeText(this,
                    "Could not verify code. If you use a Firebase test number, the account phone in the database must match it exactly (same E.164).",
    ...
    private void resendOtp() {
        FirebaseUtils.sendVerificationCode(phoneNumber, this, new Consumer<String>() {
            ...
            public void accept(String id) {
                ...
                startTimer();
                Toast.makeText(ResetPasswordOtpActivity.this, "Code sent", Toast.LENGTH_SHORT).show();
            }
        });
    }
